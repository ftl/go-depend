...
| `D` and the zone use `A` by default, and `A_edge` with the flag `--edge` | `A_edge` answers the better question for Go, but it is blind for a module that is generic from the top to the bottom, see [baseline.md](./baseline.md); a value that gates a build must not depend on such a blind spot without a decision of the user |
