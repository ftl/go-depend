...
- for an arbitrary external package, including transient dependencies, covering only incoming dependencies and packages from the module
...
- constant chagne over time -> the package is considered "instable"
- high change rate only for a certain amount of time -> the package is considered "stable"

The reality check compares the perceived instability with a package's calculated instability metric.
...
```
Scan Go packages matching the given patterns (defaults to `./...` - the whole module) and outputs a table sorted by the package name, containg the metrics and markers for violations of any invariant.

```shell
go-depend graph [pattern] [flags] 
```
Produce a dependency graph for the selected package (defautls to `.` - the package of the current working dir) and write it either to stdout or to a file (`--output filename`).
...
| `--history` | off | Reads the git history and calculates the perceived instability of each package. The table shows the perceived instability and the difference to the calculated instability. |
